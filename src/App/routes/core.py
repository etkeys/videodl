...
def authenticate():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.access_token, form.access_token.data):
            login_user(user, remember=False)
            next_page = request.args.get('next')
            print(f"Going to '{next_page}'")
            return redirect(next_page) if next_page else redirect(url_for('core.root'))
        else:
            flash('Login unsuccessful. Please check name and access token.', category='error')
